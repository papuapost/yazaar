...
YAHOO.widget.DataTable = function(elContainer,oColumnSet,oDataSource,oConfigs) {
     ...
     *
     * @event cellCancelEvent
     * @param oArgs.target {HTMLElement} The TD element.
     * @param oArgs.oldData {Object} Old data value.
     */
    this.createEvent("cellCancelEvent");
    ...
    /**
     * Fired when a Record is deleted in the RecordSet. (+)
     *
     * @event recordSetUpdateEvent
     * @param oArgs.record {YAHOO.widget.Record} The Record instance.
     * @param oArgs.key {String} The Record key.
     ...
     * @param oArgs.oldData {Object} New data.
     */
    this.createEvent("recordSetDeleteEvent");
    this._oRecordSet.subscribe("recordDeleteEvent", this._onRecordDelete, this, true);   
...
/**
 * Helper function to file cellCancelEvent. (+)
 * @private
 */
function activeEditor_hide(oSelf) {
    ...
    // Raise cellCancelEvent
    oSelf.fireEvent("cellCancelEvent",{target:elCell,oldData:oldValue});  
}
...
YAHOO.widget.DataTable.prototype._onClick = function(e, oSelf) {
    ...
    if(oSelf.activeEditor) { //&& (oSelf.activeEditor.column != column)
        // Hide active editor and raise cellCancelEvent (+)
        activeEditor_hide(oSelf);        
...
/**
 * Passes along recordSetDelete Event when recordDeleteEvent is caught from RecordSet. (+)
 *
 * @event _onRecordUpdate
 * @param oArgs.record {YAHOO.widget.Record} The Record instance.
 * @param oArgs.key {String} The Record key.
 ...
 * @private
 */
YAHOO.widget.DataTable.prototype._onRecordDelete = function(oArgs) {
    this.fireEvent("recordSetDeleteEvent",oArgs);
};
...
YAHOO.widget.RecordSet.prototype.getRecord = function(identifier) {
	if (identifier===0) return this._records[0];
    if (identifier) {
...
YAHOO.widget.RecordSet.prototype.deleteRecord = function(i, range) {
    ...
    // var oRecord = this.getRecord(i); // (+)
    var oRecord = this._records[i]; // (+)
    this.fireEvent("recordDeleteEvent",{record:oRecord,index:i}); // (+)
